                     ...
                     int       nCmdShow)
{
	...
	INITCOMMONCONTROLSEX initCommCtl = {0};
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	/*		dx = pnmLVScroll->dx;
		...
		{
			LPNMLVSCROLL pnmLVScroll = (LPNMLVSCROLL) lParam;
			if (!scrolling && pnmLVScroll->dy != 0)
			{
				scrolling = 1;
				TCHAR str[250];
				__sprintf_s(str, 250, TEXT("%d"), pnmLVScroll->dy);
				SendMessage(hList, LB_ADDSTRING, 0, (LPARAM) str); 
				int scrlDelay = min(150, 150 / abs(pnmLVScroll->dy/2));
				for (int i = 0; i < abs(pnmLVScroll->dy/2); i++)
				{
		...
		UINT i = NM_FIRST;
			...
			case CDDS_ITEMPOSTPAINT:
				...
				if (lpNMCustomDraw->dwItemType == LVCDI_ITEM)
				{
					...
					TextOut(lpNMCustomDraw->nmcd.hdc, rc.left, rc.top, str, __strlen(str));
				}
				break;
	...
	case WM_DRAWITEM:
		{
			...
			if (lpDrawItem->CtlType == ODT_BUTTON)
			{
				...
				{
					inButton = 1;
					hbmpImage = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_ON));
				}
				else
				{
					inButton = 0;
					hbmpImage = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_OFF));
				}
				...
				BitBlt(lpDrawItem->hDC, 0, 0, bm.bmWidth, bm.bmHeight, hDCMem, 0, 0, SRCCOPY);
